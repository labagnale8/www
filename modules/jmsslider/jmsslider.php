...
class Jmsslider extends Module
{
    ...
    public function install()
    {
        if (parent::install() && $this->registerHook('header') && $this->registerHook('actionShopDataDuplication')) {
            ...
            $res &=Configuration::updateValue('JMS_SLIDER_DURATION', 9000);
    ...
    public function process()
    {
        ...
        if (Tools::isSubmit('submitSlide')) {
            ...
            if ((Tools::getValue('bg_type')==1 && empty($_FILES['bg_image']['name']) && Tools::strlen(Tools::getValue('old_image'))==0
             ) ||
              ( Tools::getValue('bg_type')==0 && Tools::strlen(Tools::getValue('bg_color'))==0)) {
            ...
            for ($i=0; $i < $total_layer; $i++) {
                ...
                if (Tools::strlen(Tools::getValue('data_delay_'.$id_layer))==0 || !Validate::isInt(Tools::getValue('data_delay_'.$id_layer))
                    || Tools::getValue('data_delay_'.$id_layer)<0) {
                ...
                if (Tools::strlen(Tools::getValue('data_font_size_'.$id_layer))==0 || !Validate::isInt(Tools::getValue('data_font_size_'.$id_layer))
                    || Tools::getValue('data_font_size_'.$id_layer)<0) {
        ...
        if (count($errs)<1) {           
            if (!Tools::getValue('id_slide')) {
                ...
                $slide->order = (int)Tools::getValue('order_slide', $slide->order);
                $slide->update();                
                Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'jms_slides_lang` SET `id_lang` = '.(int)Tools::getValue('lang_slide').'
                        WHERE `id_slide` = '.$slide->id);
