...
class JmsMegaMenu extends Module
{
	...
	public function install()
	{
		if (parent::install() && $this->registerHook('header') && $this->registerHook('topcolumn') && $this->installSamples())
		{
			...
			$id_tab1 = $this->addTab('Jms MegaMenu', 'dashboard');			
	...
	public function mobile_beginmenu()
	{
		$this->mobilemenu .= '<ul class="nav nav-pills nav-stacked level0">';
	}
	
	public function mobile_endmenu()
	{
		$this->mobilemenu .= '</ul>';
	}
	...
	{	
		...
		$items = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);		
		foreach ($items as $item) {			
			$this->mobile_genItem($item['mitem_id']); 
		}	
	}
	
	public function mobile_genItem($itemid)
	{		
		$item = $this->_items[$itemid];		
		$this->context = Context::getContext();			
		$lvl     = $this->_items[$itemid]['level'];							
	...
	public function mobile_mega($itemid)
	{		
					
		$item = $this->_items[$itemid];		
		$childs = $this->children[$itemid];
		$this->mobile_beginDropdown();
		foreach ($childs as $_item)
		{
			$this->mobile_genItem($_item['mitem_id']);
		}			
		$this->mobile_endDropdown();
	}	
	public function mobile_beginDropdown()
	{		
		$this->mobilemenu .= '<ul class="dropdown-menu">';
	}
	public function mobile_endDropdown()
	{
		$this->mobilemenu .= '</ul>';
	}
