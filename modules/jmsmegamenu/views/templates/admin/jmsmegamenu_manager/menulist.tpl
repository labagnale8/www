...
<h3>
  <i class="icon-list-ul"></i> {l s='Menu List' mod='jmsmegamenu'}
	...
	<form name="adminForm" action="{$link->getAdminLink('AdminJmsmegamenuManager')|escape:'html':'UTF-8'}&configure=jmsmegamenu" method="post">
		<div id="jms-toolbar">				
				<a class="jms-tool btn-success" onclick="submitform('add','{$link->getAdminLink('AdminJmsmegamenuManager')|escape:'html':'UTF-8'}&configure=jmsmegamenu&addMenu');" title="Add New Menu">
					<i class="icon-plus"></i>
				</a>
				<a class="jms-tool btn-info" onclick="submitform('edit','{$link->getAdminLink('AdminJmsmegamenuManager')|escape:'html':'UTF-8'}&configure=jmsmegamenu');" title="Edit Menu">
					<i class="icon-edit"></i>
				</a>
				<a class="jms-tool btn-warning" onclick="submitform('deleteMenu','{$link->getAdminLink('AdminJmsmegamenuManager')|escape:'html':'UTF-8'}&configure=jmsmegamenu');" title="Delete Menus">
					<i class="icon-trash"></i>	
				</a>
				<a class="jms-tool btn-success" onclick="submitform('cStatus','{$link->getAdminLink('AdminJmsmegamenuManager')|escape:'html':'UTF-8'}&configure=jmsmegamenu&cStatus&status=1');" title="Enabled">
					<i class="icon-check"></i>
				</a>
				<a class="jms-tool btn-danger" onclick="submitform('cStatus','{$link->getAdminLink('AdminJmsmegamenuManager')|escape:'html':'UTF-8'}&configure=jmsmegamenu&cStatus&status=0');" title="Disabled">
						...
						<div class="row">
							...
							<div class="col-md-3">							
								<div class="btn-group-action pull-right">			
									...
									{/if}	
									<a class="btn {if $menu.active}btn-success{else}btn-danger{/if}" onclick="cStatus('cb{$k}','{$link->getAdminLink('AdminJmsmegamenuManager')|escape:'html':'UTF-8'}&configure=jmsmegamenu',{if $menu.active}0{else}1{/if})" title="{if $menu.active}Enabled{else}Disabled{/if}">
										<i class="{if $menu.active}icon-check{else}icon-remove{/if}"></i>
									</a>		
									<a class="btn btn-default"
										href="{$link->getAdminLink('AdminJmsmegamenuManager')|escape:'html':'UTF-8'}&configure=jmsmegamenu&editMenu&mitem_id={$menu.mitem_id|escape:'html':'UTF-8'}" title="Edit">
										<i class="icon-edit"></i>									
									</a>
									<a class="btn btn-default"
										onclick="cRemove('cb{$k}','{$link->getAdminLink('AdminJmsmegamenuManager')|escape:'html':'UTF-8'}&configure=jmsmegamenu')">
										<i class="icon-trash"></i>									
									</a>
								</div>
							</div>
