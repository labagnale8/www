...
class hotdeals extends Module {
    ...
    public function getProductSale($search_key ='')
    {
     ...
     $req = "SELECT hs.`name`, hs.`id_product`
      FROM `ps_product_lang` hs
      INNER JOIN `ps_specific_price` hss 
            ...
            $(function() {
                ...
                $mySlides.sortable({
                    ...
                    update: function() {
                        ...
                        $.post("'.$root_url.'modules/'.$this->name.'/ajax_'.$this->name.'.php?secure_key='.$this->secure_key.'", order);
                        }
                    });
    ...
    {                       
        ...
        $this->context->controller->addJS(($this->_path).'views/js/owl.carousel.js', 'all');  
